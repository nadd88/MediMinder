-- Database Schema for MediMinder
...
-- Medications table (patient-specific)
...
CREATE TABLE IF NOT EXISTS patient_caregiver (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_link (patient_id, caregiver_id),
    FOREIGN KEY (patient_id) REFERENCES users(id) ON DELETE CASCADE,
...
-- Link Sarah Tan to Nur Rashidah
INSERT INTO patient_caregiver (patient_id, caregiver_id, status) VALUES
(1, 2, 'active');

-- Medications for Sarah Tan
INSERT INTO medications (patient_id, medicine_name, dosage, frequency, schedule_time, instructions, remaining_quantity) VALUES
